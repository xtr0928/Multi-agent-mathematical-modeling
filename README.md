# “1000万以内最好的数模智能体！”
# Multi-agent Mathematical Modeling（数模管线）

> **一句话：为数学建模竞赛（MCM/ICM、华数杯等）打造的 72 小时多智能体解题管线——四模型参谋、单一拍板者定稿（新开 DeepSeek 或博士拍板）、双轨迹并行求解、SHA-512 作废重算引擎保证所有操作基于正确数据、评委视角终评 + 产物分数回归门把关后交付论文。**

输入一道竞赛题（官方题目 PDF + 数据包），输出一篇可直接提交的论文（MCM 25 页 LaTeX / 华数杯官方模板版）。全流程覆盖：**反投毒安检 → 审题 → 参谋商议 → 总设计定稿 → 双轨迹求解 → 评审 → 数值门禁 → 成文 → 检测 → 自动修复回路 → 分数回归门 → 交付**。

> **V6.1 新增（2026-09-25）S-1 反投毒摄入门禁**：所有外部输入（题目/数据集/图片）进入管线前先过安检——检测**物理隐藏投毒**（白字/不可见渲染 Tr=3/离屏/注释/元数据/附件/图片内隐藏文字…）8 类 30 项，产出净化版题目 + 投毒报告。核心命题：**文本层 ≠ 可见内容**。

## 一张图看懂（V6.1 反投毒门禁）

![V6.1 反投毒架构图](docs/V61_antipoison_arch.png)

→ docs/V61_antipoison_arch.html · 设计文档 docs/V61_antipoison_design.md

（V5.2.1 总架构：docs/V521_overall_arch.html ｜ V5.2 旧图：docs/V52_overall_arch.html）

## 完整工作流（V6.1，S-1–S7）

```
S-1 反投毒摄入门禁（V6.1 ★：题目/数据集/图片 → 净化版 + 投毒报告；0 LLM，fail-closed）
  → S0 问题解析 → S1 参谋组(四模型只交候选方案，不产定稿) + 分歧清单
  → S1.5 总设计定稿（单一拍板者 ★）
        自动 = 新开 DeepSeek 实例（零上下文防路径固执）
        半自动 = 问博士拍板
  → S2 按定稿求解（定稿即法律，不许自创方法/参数/口径）
  → S3 四模型评审 → S3 投票 → S4 数值门禁(0 LLM，确定性)
  → S5 排版(视觉官) → S5b 检测层作者回避抽取
  → S6 自动修复回路（修订单→repair→再评审，直到收敛）
  → S7 产物分数回归门（v2.8 评审分数 ≥ 基线，否则阻断交付）
```

完整场次 LLM 调用预估 **30 次**（S0:1 + S1:4参谋+1分歧+1假设官+1DA + S1.5:1拍板 + S2:2 + S3:4+1 + S4:0 + S5:1 + S5b:12 + S6:0确定性 + S7:0确定性 + 交付:1）。

### 72 小时竞赛时间窗

| 阶段 | 时间 | 占比 | 说明 |
|---|--:|:--:|---|
| 0 进气 + 1 结构化 | 3h | 4% | 快，不纠结 |
| 2 参谋 + 拍板定稿 | 10h | 14% | **深度的主场**：四模型参谋深推 → 分歧清单 → 单一拍板者定稿（自动=新开 DS / 半自动=问博士），reasoning 拉满 |
| 3 双轨迹执行 | 30h | 42% | 计算主体（含验证产物） |
| 4 产物级评审 | 4h | 6% | 有产物可看，高效 |
| 5 成文 | 18h | **25% 硬保底** | 纠错循环永远不得侵蚀 |
| 6-7 一致性+对抗 | 7h | 10% | 超限带风险交付，不消内容保平安 |

## 核心机制（它凭什么保证质量）

1. **参谋组 + 单一拍板者（V5.2）**：DeepSeek/GLM/Kimi K3/Qwen3.8-Max 四个不同模型家族只做**参谋**——独立深推交候选方案 + 分歧清单（冲突点显式列出），**不产定稿**；定稿由单一拍板者拍板：自动 = 新开 DeepSeek 实例（零上下文，防路径固执），半自动 = 博士确认。**定稿即法律**——后续阶段不得自创方法/参数/口径。多视角是资产，但方案所有权必须归一（v5.1 四模型商议无人拍板的教训）。
2. **双轨迹 A/B 并行**：从商议方案中选最强两条独立路线并行执行，产出后投票选主线。不把鸡蛋放在一个模型的一条思路上。
3. **声明注册表（claims registry）**：散文不是主产物，注册表才是。每条数值/结论声明带 scope 四元组（口径/群体/时代/规则）、provenance（结果仓路径+key）、依赖关系——①同名同口径不同值 = 脚本阻断；②派生量必须能反算（149/259=57.53% ✓）；③caveat 沿依赖图强制传播（§5 承认的局限不许在 §7 被静默丢弃）；④强度分级（descriptive/associational/causal/validated）与证据类型（replay/holdout/simulation/external）的越级规则表。
4. **检测层四模型独立抽取 + 作者回避**：四个不同家族独立抽取全文声明取并集（漏抽模式不相关）；写 §N 的实例不得参与 §N 的抽取裁决；种子矛盾库注入式回归测试（召回率是硬指标，漏检自动入库归因）。
5. **验证产物强制**：每条路线立项时必须声明配套验证——反演→模拟恢复实验；预测→holdout；优化→约束审计+下界。哈希链只防篡改不防错跑，**验证产物才是「跑对了」的证据**。
6. **确定性一致性层**：hash/manifest/四方核对/格式断言全部脚本化并挂 build 钩子（不靠 Agent 自觉）。否决权只留给造假类（编造数据/文献/版本号/漏问）；其余问题降级为 risk_report 随交付附出。
7. **评委 skill v2.8 终评（四评 + 全量数据复算）**：GLM/Kimi/新开 DeepSeek 三个**零撰写上下文**实例全权评审 + Qwen3.8-Max 第 4 评（校准阶梯）+ 扣分记录制评分引擎 + 真人常模分位映射 + 四方数字 diff + **全量数据复算闭环**（按论文逻辑重算每个注册数字；复算不符先四重自查评审脚本，脚本无问题才标记论文错误）。排版检查全交 Qwen 视觉官。
8. **作废重算引擎（V5.1 新增，数据正确性铁律）**：六类节点依赖 DAG + SHA-512 三级新鲜度链——任何上游修改（数据/脚本/参数）→ 传递闭包下游全部作废重算；写入门卫拦截旧进程回灌；原子发布+断点恢复。**所有操作基于正确的数据，保证每个数字可复现。**
9. **C++ OpenMP 确定性执行器（V5.1 新增，算力优势）**：固定分块+pairwise 归约+counter-based RNG，实测 2/8/16 线程结果逐位一致；文件队列+编译缓存+Python 回退。
10. **假设官 + Devil's Advocate**：假设生命周期四步（登记→必要性→挑战→敏感度）+ 隐式假设映射表；DA 专职挑刺（反事实+实验设计强制）。
11. **反投毒摄入门禁（V6.1 ★，`pipeline/ingest/`）**：8 类 30 项确定性检测，**零 LLM 成本**——视觉隐藏（对比度/**渲染模式 Tr=3**/透明度/离屏）· 载体隐藏（注释/元数据/XMP/书签/图层/表单/**附件内容**/JS）· 字符级（零宽/方向控制/同形字）· 结构级（原始字节流/增量更新）· 图片级（像素级对比度+**形态学去噪**+纯度判据）· 数据集（公式注入/隐藏 sheet·行·列/**白字**/**批注**/定义名称/**宏**）· 语义级（多特征加权评分）。**四核心机制**：三通道差分（原始 − 净化 = 证据）· **像素交叉验证**（文本层说有字→渲染上真有笔画吗）· **双轨判定**（物理隐藏 vs 语义可疑，修误报核心）· **纯度判据**（区分隐藏文字与图表）。

## v5.2.1 新增（2026-08-16，华数杯 C 四轮评审实战校准）

1. **S2.5 实现-定稿比对门禁**（`pipeline/gate/design_fidelity_gate.py`，0 LLM，fail_closed）：求解后 `impl_manifest.json` 逐条声明 D-决策实现架构，与定稿不符且未引用兜底条款编号 = 阻断。堵"定稿时拍板、实现时打折"（Q4 联合优化静默降级串行的教训）。
2. **装配纪律 R1-R6**（`pipeline/writing/assembly_rules.py`）：对照表→结论直接导出（禁止反向圆说）；头条结论与结构性前提同级；恒等式禁止包装为命题；可疑精确必附机制；方法对比含数据形状适配基线。
3. **决策核心适配论证**（orchestrator ROLE_PROMPTS）：参谋必答「数据形状→建模粒度」+「数据字典条款→口径映射」，缺一废票；拍板者定稿含口径条款清单+兜底协议编号。
4. **评委 skill v2.9**（`skills/data-science/mathmodel-judge-perspective/`）：D4 声明强度 / D5 可疑精确性 / M7 MCDA 稳健性 / M8 基线完备性 四审计。

## v6.1 新增（2026-09-25，S-1 反投毒摄入门禁）

1. **`pipeline/ingest/poison_detector.py`**（753 行）：8 类 30 项确定性检测器。主数据源改用 `get_texttrace()`——`type` 字段 = render mode，**是唯一能拿到 `Tr=3`（不可见渲染）的路径**。
2. **`pipeline/ingest/anti_poison_gate.py`**：门禁封装（fail-closed）+ 三件套产出（`clean_problem.md` / `poison_report.md` / `ingest_manifest.json`）+ **SHA-512 四哈希链**（files/clean/findings/report，含阈值与版本号——**阈值改动 = 结论变更，可追溯**）。
3. **`pipeline/orchestrator.py`** 新增 `s_1_ingest_gate()`；`run_full(..., ingest_paths=...)` 可选接入（不传则向后兼容跳过）。
4. **`pipeline/tests/test_anti_poison.py`**：G1-G8 共 **26 项验收全过**；`tests/fixtures/attack/` 沉淀 16 种攻击手法样本（防回归）。

**实测数据**：攻击样本 16 项检出 **15（94%）** · 官方真实语料 **41 文件零误报** · 零 API 成本。

**⚠️ 赛期决定风险窗口**：赛中赛题受保护、无第三方整理版 → 投毒风险 ≈0；**赛后「全套题目+优秀论文」包涌现时才是风险期**。

## 模型角色矩阵

| 角色 | 模型 |
|---|---|
| 建模手 ×4（S2 双轨迹） | DeepSeek V4 Pro / GLM 5.3 / **Kimi K3** / **Qwen3.8-Max** |
| 假设官 / Devil's Advocate | 独立实例（作者回避系统强制） |
| 反思器（完成度检查+hot_loop 推荐） | 确定性为主 + 独立实例 |
| S3 评审 ×4（相对排序）+ 投票 | DeepSeek / GLM / Kimi / Qwen |
| 检测层抽取 ×4（作者回避：写 §N 不抽 §N） | DeepSeek / GLM / Kimi / Qwen |
| 评委终评 v2.8 四评（零撰写上下文 + 扣分制 + 分位映射 + 全量数据复算） | GLM / Kimi / DeepSeek 全权 + Qwen3.8-Max（校准阶梯） |
| 视觉核查 + 排版检查 | Qwen3.8-Max 视觉官（排版检查全交千问） |
| 确定性层（作废引擎/门禁/C++ 执行器/状态机） | 纯脚本，0 LLM，挂 build 钩子 |

## 实战记录（产物全部在 docs/，可追溯）

| 场次 | 产物 | 要点 |
|---|---|---|
| 2026 MCM C（DWTS 淘汰预测） | `docs/DWTS_2026C_v5.pdf`（15 页 LaTeX）+ `docs/v5_run/` 全部运行产物 | V5 首战：registry 种子回归 5/5、四方核对 34/34、模拟恢复实验证伪「回代 100%」并如实写进论文 |
| 华数杯 2026C（算电协同调度） | 12 页官方模板版 | 调度贪心三原则、C++ OpenMP 重写、消融归因 |
| 55 题真题批测 | 技能库基准文档 | 平均 80.4（⚠️ 子代理自评口径，见下） |

## 仓库结构

```
pipeline/                           # V5.2 可执行管线（参谋组+拍板者+修复回路+回归门；tests 9/9+9/9）
  engine/                           #   作废重算引擎：SHA-512 哈希/DAG/registry 写入门卫/传播调度
  cpp/                              #   C++ OpenMP 执行器：队列/编译缓存/回退/KS 对照/确定性模板
  gate/  state/  assumptions/       #   数值门禁四断言 / 双状态机 / 假设官
  reflect/                          #   反思器 + Devil's Advocate
  layout/  writing/  audit/         #   排版层 / 微单元模板 / 角色隔离审计
  seed/  bench/  visual/  baseline/ #   种子库17条 / 内部基准 / DAG可视化 / O奖基线
  ingest/                           #   V6.1 S-1 反投毒门禁（8类30项检测器 + fail-closed 封装 + 哈希链）
  orchestrator.py                   #   LLM 编排层 S-1+S0-S6（DryRun 零成本验证）
  PROJECT.md                        #   详细项目文档（总体架构/验收表/铁律）
skills/
  data-science/
    1start-mathmodel … 6verity/     # 6 阶段论文流水线
    math-brainstorm/  brute-force-think/   # 思路脑暴 / 暴力求解
    mathmodel-judge-perspective/    # 评委视角 skill v2.9（权威版）
    mathmodel-figure-templates/  typst-author/  doctor/  _references/
  multi-agent-pipeline/             # 6 阶段编排 + 架构全景
docs/
  V5_pipeline_design.md             # V5 管线完整设计（含设计决策溯源表）
  V5_upgrade_dev_doc_2026-08-15.md  # V5.1 升级开发文档（混合编码管线产出）
  V52_pipeline_design.md            # V5.2 设计定稿（参谋组+单一拍板者+产物回归门，2026-08-15）
  V52_overall_arch.html/.png        # V5.2 总架构图（纸墨风 9 泳道）
  V5_optimization_plan_2026-08-15.html  # 优化清单（16 项→15 项实施决策）
  V521_overall_arch.html/.png       # V5.2.1 总架构图（S2.5 比对门禁+装配纪律 R1-R6+评委 v2.9 四审计）
  V61_antipoison_design.md          # V6.1 反投毒门禁设计文档（检测矩阵/四核心机制/7 轮误报抑制实录）
  V61_antipoison_arch.html/.png     # V6.1 反投毒架构图（纸墨风 6 泳道）
  2026_MCM-ICM_Problems/            # 2026 官方题目包
  v5_run/                           # V5 首战代码与全部运行产物
```

## 版本时间线（git tag，完整演进）

| tag | 里程碑 |
|---|---|
| `v2.2` | 融合架构 v2.2（mathmodel-v2-pipeline 初版：模式选择/建模辩论/数值验证门禁/五人评审团） |
| `pipeline-v2.3` `pipeline-v2.4` | C 题 O 奖论文精读驱动的迭代（优缺点强制/灵敏度标配/多模型对比） |
| `pipeline-v3.0` | 全题型版：437 篇 O 奖精读 → 4 条 100% 全票铁律 + 8 条高票规则 |
| `pipeline-v3.1` | 55 题批测驱动升级（交付优先序/图密度/灵敏度矩阵/数据辩护模板） |
| `v5-pipeline` | V5 管线全链路并入：三 Agent 商议引擎 / claims registry / 双轨迹实战 / 15 页论文 |
| `judge-v1.0` … `judge-v1.6` | 评委 skill 依据审计时代：官方 Triage 指南 → 84 篇 C 题逐篇精读 |
| `judge-v2.0` `judge-v2.1` | 437 篇全题型校准（HARD FAIL 扩至 10 项） |
| `judge-v2.3` | 两段式评审（确定性审计前置） |
| `judge-v2.5` `judge-v2.5.1` | 六阶段架构 + 三评仲裁 + 视觉评审层（冷启动五层参照系） |
| `judge-v2.6` | 终版：四方数字 diff + 名实审查 E1-E4 + 扣分记录制评分引擎 |
| `judge-v2.7` | 四评版：+Qwen3.8-Max 校准阶梯第 4 评；排版检查全交千问；四建模手（Kimi K3/Qwen3.8-Max） |
| `judge-v2.8` | **全量数据复算闭环**：按论文逻辑重算每个数字；复算不符先四重自查评审脚本，脚本无问题才标记论文错误 |
| `pipeline-4model-design` | 四模型化设计存档（Qwen 第四全权模型，2026-08-15 已实施） |
| `v5.1-pipeline-core` | V5.1 管线核心：作废重算引擎/C++ 确定性执行器/数值门禁/假设官/排版层，14 项验收 |
| `v5.1-pipeline-full` | **V5.1 全功能版**：+反思器/DA/双轨迹/检测层作者回避/KS 对照/编排层 S0-S6，32/32 验收 |
| `v5.2-pipeline` | **V5.2 单一拍板者版**（2026-08-15）：S1 降为参谋组+分歧清单；S1.5 拍板者（自动=新开 DS / 半自动=问博士）；S6 自动修复回路；S7 产物分数回归门；六条铁律入 PROJECT.md |
| `v5.2.1` | **V5.2.1 定稿即法律闭环**（2026-08-16）：S2.5 实现-定稿比对门禁 + 装配纪律 R1-R6 + 评委 v2.9 四审计 |
| **`v6.1`** | **S-1 反投毒摄入门禁**（2026-09-25）：8 类 30 项确定性检测（含 **Tr=3 不可见渲染**、附件内容读取、图片像素级隐藏文字）· 四核心机制（三通道差分/**像素交叉验证**/**双轨判定**/**纯度判据**）· fail-closed + SHA-512 四哈希链 · 26/26 验收 · 攻击样本检出 94% / 官方 41 文件零误报 |

## 诚实边界（重要，引用本仓库数字前请读）

0. **反投毒门禁（V6.1）只覆盖物理隐藏类投毒**——白字/不可见渲染/离屏/零宽/载体隐藏/图片像素级隐藏。
   **语义投毒**（内容正常但有误导性）无物理特征，规则抓不到，需 LLM 语义层（尚未实现）。
   **MinerU OCR 通道未实现**（像素交叉验证已取代其"判定可见性"职能）。
   另：**赛期决定风险窗口**——赛中赛题受保护、无第三方整理版；赛后"全套题目+优秀论文"包涌现时才是风险期。
1. **55 题批测平均 80.4 是子代理自评口径**，非外部评审——两者必须分开标注，不可混用。
2. 评委 v2.6 首战：三评中位数与真人 80 分锚偏差 +3.5（达标）；**分位映射层尚未在评审管线中完全接线**，绝对分校准仍在进行。
3. 「多模型互证 16/16」等铁律来自 437 篇 **O 奖正样本**精读——正样本无对照组，属待验证假设（M/H 奖对照扫描是计划项）。
4. 四模型化升级与 V5.1 管线（2026-08-15 用户指令）**已实施**：四建模手（Kimi 建模手=K3、+Qwen3.8-Max）、四模型评审、四模型检测；评委四评走校准阶梯；**V5.1 可执行管线（pipeline/，32/32 验收）**：SHA-512 作废重算、C++ 确定性执行、数值门禁、假设官、DA、反思器、双轨迹、检测层作者回避。剩余判据（A3/A7/A9-A12/A21/A25/A27/A29-A31）待真实竞赛运行验证。
5. **V5.2 单一拍板者升级（2026-08-15 夜，v2.8 评审实战驱动）已实施**：华数杯 C 题 v5.1 产物经 v2.8 评审 47/100 → 修订回路修复至 50，暴露根因"过程治理≠产物正确；四模型商议≠有人负责"。V5.2 三反转：建模时间前置、方案所有权收拢（参谋组+单一拍板者）、产物分数进门禁（S7 回归门）。华数杯 C 基线分数 = 50，后续版本分数低于此值阻断交付。
6. 论文语料库（437 篇 O 奖 PDF 全量精读笔记）在本地技能库中，不在本仓库。

## 与协同编码管线的接口

- 本仓库**不依赖**任何编码管线 skill；Stage 3 的计算需求（调度器/扫描器/管线脚本等）作为任务派发给协同编码管线执行。
- 通用铁律：写评分离、确定性门禁不认模型、72h 时间窗、评审零撰写上下文实例隔离、旧进程不杀并行对照。
- 三仓库分家说明：`docs/pipelines-split-2026-08.md`。

## 相关仓库

- 协同编码管线 → [Multi-agent-programming-pipeline](https://github.com/xtr0928/Multi-agent-programming-pipeline)
- 通用工具与编排 → [Multi-Agent](https://github.com/xtr0928/Multi-Agent)
