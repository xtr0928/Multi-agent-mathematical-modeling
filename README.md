...
Stage 2 四 Agent 商议  四个建模手（DeepSeek/GLM/Kimi K3/Qwen3.8-Max）独立深推 → 交换批评 → 各自修订（2-3 轮自然停止，分歧不强制收敛）
Stage 3 双轨迹执行  最强两条路线 A/B 并行真实求解（含验证产物、基线对照、灵敏度矩阵）
Stage 4 产物级评审  四模型独立评审 + 投票选主线（LLM 只做相对排序，不做绝对打分）
...
7. **评委 skill v2.7 终评（四评）**：GLM/Kimi/新开 DeepSeek 三个**零撰写上下文**实例全权评审 + Qwen3.8-Max 第 4 评（校准阶梯：影子 ≥15 有效轮 → 灰度 5 轮 → 转正；四票仲裁三规则 + 同源偏移检测）+ 扣分记录制评分引擎 + 真人常模分位映射 + 四方数字 diff（正文/表格/图内/摘要全局比对）。排版检查全交 Qwen 视觉官。

## 模型角色矩阵

| 角色 | 模型 |
|---|---|
| 建模手 ×4（Stage 2 商议） | DeepSeek V4 Pro / GLM 5.2 / **Kimi K3** / **Qwen3.8-Max** |
| 检测层抽取 ×4（作者回避） | DeepSeek / GLM / Kimi / Qwen |
| Stage 4 评审 ×4（相对排序） | DeepSeek / GLM / Kimi / Qwen |
| 评委终评 v2.7 四评（零撰写上下文 + 扣分制 + 分位映射） | GLM / Kimi / DeepSeek 全权 + Qwen3.8-Max（校准阶梯：影子≥15 有效轮→灰度 5 轮→转正） |
| 视觉核查 + 排版检查（渲染检查/图内文字提取/版面门禁视觉部分） | Qwen3.8-Max 视觉官（排版检查全交千问） |
...
4. 四模型化升级（2026-08-15 用户指令）**已实施**：四建模手（Kimi 建模手=K3、+Qwen3.8-Max）、四模型评审、四模型检测；评委四评走校准阶梯（Qwen 影子期起跑）；设计存档 `docs/pipeline_4model_design.md`。
